...
# :fontawesome-brands-java: Java {{ java_8.jdk_standerd.config.version }} LTS
</div>
...
???+ info "Adoptium Upstream Release Metadata"
    ...
    | :--- | :--- |
    | **Full Version** | `{{ java_8.jdk_standerd.config.full_ver }}` |
    | **SemVer** | `{{ java_8.jdk_standerd.config.semver }}` |
    | **Security Level** | `psu-{{ java_8.jdk_standerd.config.sec_level }}` |
    | **Upstream Update** | `⏱️ {{ java_8.jdk_standerd.config.upstream_update }}` |
...
=== ":{{ icon }}: {{ title }}"
    ...
    **Pull by Version Tag**
    ```bash
    docker pull {{ java_8[key].tags[0] }}
    ```

    **Pull by Floating Tag**
    ```bash
    docker pull {{ java_8[key].tags[1] }}
    ```

    **Pull by Immutable Digest (Recommended)**
    ```bash
    docker pull {{ repo_of_registry }}@{{ java_8[key].main.provenance.digest }}
    ```

    <p class="integrity-meta">
        Integrity Metadata: :material-file-certificate: [L3 Provenance]({{ java_8[key].main.provenance.url }}) | :material-barcode-scan: [CycloneDX SBOM]({{ java_8[key].main.sbom.url }})
    </p>
    ...
    === ":material-shield-search: Vulnerability Scan"
        ...
        {% set scan_data = java_8[key]['security'] %}
        {% set sbom_url = java_8[key]['main']['sbom']['url'] %}
    ...
    === ":fontawesome-brands-docker: Docker CIS"
        ...
        {% set cis_data = java_8[key]['docker-csi'] %}
    ...
    === ":material-kubernetes: K8s NSA"
        ...
        {% set nsa_data = java_8[key]['k8s-nsa'] %}
    ...
    === ":material-lock-check: K8s PSS"
        ...
        {% set pss_data = java_8[key]['k8s-pss-restricted'] %}
